...
const SCOPE_NEXT_EVIDENCE = {
  ...
  response_audit: [
    'Correct the flagged statements and re-run the audit',
    'Verify the corrected summary against the original governance block'
  ]
};
...
const SCOPE_CANNOT_CLAIM = {
  ...
  response_audit: [
    'Do not say the underlying accessibility finding has been re-tested or verified.',
    'Do not treat a clean audit as evidence the page is accessible.',
    'Do not say this audit covers claims outside the supplied summary text.',
    'Do not say the summary author intended to mislead.'
  ]
};

// A pass result is the easiest finding to over-claim from, so it gets
// additional explicit prohibitions on top of the scope list.
const PASS_EXTRA_CANNOT_CLAIM = [
  'Do not say the component is WCAG compliant.',
  'Do not say the page passes accessibility.',
  'Do not say rendered behaviour has been tested.'
];
...
function buildCanClaim(result, scope) {
  ...
  if (scope === 'response_audit') {
    const v = result.violations?.length ?? 0;
    if (decision === 'pass') return 'The drafted summary stays within the claim boundary of the audited finding.';
    if (decision === 'fail') return `The drafted summary contains ${v} statement(s) that drift beyond the audited finding's claim boundary.`;
    return `The drafted summary has ${v} consistency concern(s) requiring revision before use.`;
  }
...
function buildReleaseGate(result) {
  ...
  const blockingFindings = failures.map(issue => ({
    criterion: (issue.criteria || ['unknown'])[0] || 'unknown',
    reason: issue.message || issue.suggested_fix || 'Deterministic failure detected.'
  }));

  // A blocking gate must always carry at least one blocking finding for audit
  // clarity. Tools that fail without an issues list (e.g. the contrast
  // calculator) get a finding synthesised from their criteria and answer.
  if (decision === 'fail' && !blockingFindings.length) {
    const ids = (result.criteria || []).map(c => (typeof c === 'string' ? c : c.id)).filter(Boolean);
    blockingFindings.push({
      criterion: ids[0] || 'unknown',
      reason: result.answer || 'Deterministic failure detected.'
    });
  }
...
export function buildGovernance(result, scope) {
  const decision = result.decision || 'not_applicable';
  const cannotClaim = SCOPE_CANNOT_CLAIM[scope] || SCOPE_CANNOT_CLAIM.wcag_reference;
  return {
    ...
    claim_boundary: {
      ...
      cannot_claim: decision === 'pass' ? [...new Set([...cannotClaim, ...PASS_EXTRA_CANNOT_CLAIM])] : cannotClaim
    },
