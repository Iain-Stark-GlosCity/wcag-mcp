...
import { FAILURE, WARNING, NOTE, summariseIssues, decisionFor } from './issueClassification.js';
...
function addIssue(issues, issue) {
  issues.push({ severity: 'medium', classification: WARNING, criteria: ['4.1.2'], ...issue });
}
...
export function validateAriaAttributes({ html = '', target_level = 'AA', context = '', output_mode } = {}) {
  const issues = [];
  ...
  for (const el of root.querySelectorAll('*')) {
    for (const attr of el.getAttributeNames()) {
      if (attr.startsWith('aria-') && !KNOWN_ARIA.has(attr)) {
        addIssue(issues, { severity: 'high', classification: FAILURE, selector: selectorFor(el), attribute: attr, message: `${attr} is not a recognised ARIA attribute.`, suggested_fix: 'Remove the attribute or replace it with the correct ARIA attribute name.' });
      }
    }
    ...
    if (el.hasAttribute('aria-expanded')) {
      ...
      if (!['true', 'false'].includes(value)) {
        addIssue(issues, { severity: 'high', classification: FAILURE, selector: selectorFor(el), attribute: 'aria-expanded', message: 'aria-expanded must be true or false when present.', suggested_fix: 'Set aria-expanded="false" when collapsed and "true" when expanded.' });
      }
      if (!el.matches(INTERACTIVE_SELECTOR)) {
        addIssue(issues, { severity: 'high', classification: FAILURE, selector: selectorFor(el), attribute: 'aria-expanded', message: 'aria-expanded is on an element that is not natively interactive and has no recognised interactive role.', suggested_fix: 'Put aria-expanded on the button or interactive trigger that opens the content.' });
      }
      if (!el.hasAttribute('aria-controls')) {
        addIssue(issues, { classification: NOTE, selector: selectorFor(el), attribute: 'aria-expanded', message: 'Disclosure-style controls with aria-expanded should identify the controlled content.', suggested_fix: 'Add aria-controls pointing to the id of the disclosed region when that relationship is useful to expose.' });
      } else if (!elementWithId(root, el.getAttribute('aria-controls'))) {
        addIssue(issues, { severity: 'high', classification: FAILURE, selector: selectorFor(el), attribute: 'aria-controls', message: 'aria-controls points to an id that was not found in the snippet.', suggested_fix: 'Ensure aria-controls exactly matches the controlled region id.' });
      }
      if (!hasStateChangeHook(el)) {
        addIssue(issues, { classification: NOTE, selector: selectorFor(el), attribute: 'aria-expanded', message: 'aria-expanded is present but no state-change handler or framework hook is visible in the snippet.', suggested_fix: 'Wire the trigger so activation toggles both the visual state and aria-expanded.' });
      }
    }
    ...
    if (el.tagName.toLowerCase() === 'img' && el.hasAttribute('alt') && el.hasAttribute('aria-label')) {
      addIssue(issues, { classification: WARNING, selector: selectorFor(el), attribute: 'aria-label', message: 'Image has both alt and aria-label, which can create redundant or conflicting accessible names.', suggested_fix: 'Use alt for the image accessible name; remove aria-label unless there is a specific tested reason.' });
    }

    const role = el.getAttribute('role');
    if (role === 'combobox' && !el.hasAttribute('aria-expanded')) {
      addIssue(issues, { severity: 'high', classification: FAILURE, selector: selectorFor(el), attribute: 'aria-expanded', message: 'Comboboxes need an aria-expanded state.', suggested_fix: 'Expose aria-expanded and update it when the popup opens or closes.' });
    }
    if (role === 'tab' && !el.hasAttribute('aria-selected')) {
      addIssue(issues, { severity: 'high', classification: FAILURE, selector: selectorFor(el), attribute: 'aria-selected', message: 'Tabs need aria-selected to expose the active tab.', suggested_fix: 'Set aria-selected="true" on the active tab and "false" on inactive tabs.' });
    }
    if (role === 'tabpanel' && !el.hasAttribute('aria-labelledby') && !el.hasAttribute('aria-label')) {
      addIssue(issues, { classification: WARNING, selector: selectorFor(el), attribute: 'aria-labelledby', message: 'Tab panels need an accessible name, normally from the associated tab.', suggested_fix: 'Add aria-labelledby pointing to the owning tab id.' });
    }
  }
  ...
  const result = withSourceMetadata({
    ...
    criteria,
    issues,
    failures: issues.filter(i => i.classification === FAILURE),
    warnings: issues.filter(i => i.classification === WARNING),
    notes: issues.filter(i => i.classification === NOTE),
